...
ILP_print (ILP_Object self)
{
     if ( self->_class == &ILP_object_Integer_class ) {
          ...
          fprintf(stdout, "%.5g", self->_content.asFloat);
...
/*tme 8*/
ILP_Object*
ILP_lookup_property(ILP_Object target, char* property)
{
     ILP_Class clazz = target->_class;
     ILP_Field field = clazz->_content.asClass.last_field;
     while(field){
          if(!strcmp(field->_content.asField.name, property)){
               short offset = field->_content.asField.offset;
               return & (target->_content.asInstance.field[offset]);
          }
...
ILP_Object
ILP_read_property(ILP_Object property, ILP_Object target)
{
...
ILP_Object
ILP_write_property(ILP_Object property, ILP_Object target, ILP_Object value)
{
     ILP_CheckIfString(property);
     ILP_Object* field = ILP_lookup_property(target, property->_content.asString.asCharacter);
     if(!field){
...
ILP_Object
ILP_has_property(ILP_Object property, ILP_Object target)
{
     ILP_CheckIfString(property);
     ILP_Object* field = ILP_lookup_property(target, property->_content.asString.asCharacter);
     return field ? ILP_TRUE : ILP_FALSE;
}
